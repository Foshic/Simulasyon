...
Logger[] loggers = {new MailLogger(), new DatabaseLogger(), new FileLogger()};
		...
		courses.add(new Course(2,"Php","Programlama","xxxx",150));
		courses.add(new Course(3,"Photoshop","Tasarım","xxx",150));

		CourseManager courseManager = new CourseManager(new HibernateCourseDao(), loggers, courses);
		
		try {
			courseManager.add(new Course(5, "C++","Programlama","xxxxxx",-3));
		}catch(Exception e){
			System.out.println(e);
		}
